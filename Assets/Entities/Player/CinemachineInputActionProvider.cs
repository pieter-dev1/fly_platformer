...
namespace Cinemachine
{
    ...
    /// </summary>
    public class CinemachineInputActionProvider : MonoBehaviour, AxisState.IInputAxisProvider
    {
        /// <summary>
        ...
        public virtual float GetAxisValue(int axis)
        {
            if (enabled)
            {
                var action = axis == 2 ? ZAxis : XYAxis;
                if (action != null)
                {
                    switch (axis)
                    {
